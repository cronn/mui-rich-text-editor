...
function renderToolbar(overrides: Partial<Parameters<typeof EditorToolbar>[0]> = {}, editor: Parameters<typeof EditorToolbar>[0]["editor"]) {
...
describe("EditorToolbar", () => {
    it("renders all toolbar buttons by default", async () => {
        ...
        expect(screen.getByRole("button", { name: "Link hinzufügen" })).toBeInTheDocument();
    ...
    it("hides the image upload button when disableImageUpload is true", async () => {
        ...
        expect(screen.queryByTestId("rich-text-image-upload")).not.toBeInTheDocument();
    });
});
