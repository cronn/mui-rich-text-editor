...
describe("useCustomEditor", () => {
    ...
    it("sets and unsets font color", async () => {
        ...
        expect((editor.getAttributes("textStyle") as { color?: string }).color).toBe("#ff0000");

        runInAct(() => {
            editor.chain().focus().unsetColor().run();
        });
        expect((editor.getAttributes("textStyle") as { color?: string }).color).toBeUndefined();
    });
    ...
    it("sets font size", async () => {
        ...
        expect((editor.getAttributes("textStyle") as { fontSize?: string }).fontSize).toBe("20px");
    });
    ...
    describe("customImage node", () => {
        it("insertCustomImage inserts an image node with given attributes", async () => {
            ...
            runInAct(() => {
                editor.commands.insertCustomImage({ src: "https://example.com/a.png", width: "100", height: "50" });
            });
        ...
        it("insertCustomImage defaults align to 'center' when omitted", async () => {
            ...
            const imageNode = doc.content?.find((node) => node.type === "customImage");
        ...
        it("setImageAlign updates align attribute when selection is on the image", async () => {
            ...
            runInAct(() => {
                editor.commands.insertCustomImage({ src: "https://example.com/a.png", align: "left" });
            });
        ...
        it("parses width/height from a raw <img> tag", async () => {
            const { editor } = await renderEditor({
                content: '<img src="https://example.com/raw.png" width="30" height="20" />',
            });
        ...
        it("parses align='center' from inline margin:auto styles on a raw <img> tag", async () => {
            const { editor } = await renderEditor({
                content: '<img src="https://example.com/raw.png" style="margin-left:auto;margin-right:auto;" />',
            });
        ...
        it("parses align='right' from inline margin-left:auto;margin-right:0px styles", async () => {
            const { editor } = await renderEditor({
                content: '<img src="https://example.com/raw.png" style="margin-left:auto;margin-right:0px;" />',
            });
