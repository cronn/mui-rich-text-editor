...
function getMuiInputProps(
  props: TextInputProps,
): Partial<OutlinedInputProps> | undefined {
  ...
  return {
    ...
    ...(loading !== undefined && {
      endAdornment: <LoadingAdornment loading={loading} />,
    }),
  };
}

export type TextControlProps<TFormValues extends FieldValues> =
  CustomFormControlProps<TFormValues> & TextInputProps;
...
type TextInputProps = NativeInputProps & {
  ...
  setCursorPosition?: (
    element:
      (EventTarget & HTMLInputElement) | (EventTarget & HTMLTextAreaElement),
  ) => void;
  onFocus?: (
    element: FocusEvent<HTMLInputElement | HTMLTextAreaElement>,
  ) => void;
...
export function TextControl<TFormValues extends FieldValues>(
  ...
  function formatDisplayedValue(value: string) {
    return isDefined(props.formatDisplayedValue)
      ? props.formatDisplayedValue(value)
      : value;
  }
